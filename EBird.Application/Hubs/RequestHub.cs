    ...
    public class RequestHub : Hub
    {
        ...
        public override async Task OnConnectedAsync()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(groupIdRaw))
                    groupIdRaw = Context.GetHttpContext().Request.Query["roomId"].ToString().ToLower();
        ...
        public async Task SendRequestGroup(RequestCreateDTO requestDto)
        {
            var groupId = OnlineUsers.FirstOrDefault(x => x.Value.Contains(Context.ConnectionId)).Key.GroupId;

            requestDto.GroupId = groupId;
            requestDto.HostId = Context.User.GetUserId();
            if (requestDto == null)
            {
            ...
            var rowEffect = await _repository.Request.CreateAsync(requestEntity);

            if (rowEffect == 0)
                throw new BadRequestException("Request cannot be created");
        }

        public async Task SendRequestRoom(RequestCreateDTO requestDto)
        {
            var roomId = OnlineUsers.FirstOrDefault(x => x.Value.Contains(Context.ConnectionId)).Key.GroupId;

            requestDto.RoomId = roomId;
            requestDto.HostId = Context.User.GetUserId();
            if (requestDto == null)
            {
                throw new BadRequestException("Request cannot be null");
